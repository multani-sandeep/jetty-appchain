...
#Start Hybris-AD
set +x
#java $debugAD  $appdAD -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8181 "$APPCHAIN_HOME"/target/test.war > /tmp/server.log &

sleep $APP_START_DELAY
#Start Hybris-ACP
java $debugACP $appdACP -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8282 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &

sleep $APP_START_DELAY
#Start EI-ESB
java $debugESB $appdESB -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8383 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &

sleep $APP_START_DELAY
#Start EI-AMQ
#java $debugAMQ $appdAMQ -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8484 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &

sleep $APP_START_DELAY
#Start AbstractionLayer

java $debugABL $appdABL -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8585 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &

sleep $APP_START_DELAY
#Start CustomerPayments.Gateway
#java $debug $appdCPG -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8686 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &
...
sleep $APP_START_DELAY
#Start API.Gateway
java  $jettyAPIGWY -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8888 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &

sleep $APP_START_DELAY
#Start B2B
java $debugB2B $appdB2B -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 8989 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &
...
sleep $APP_START_DELAY
#Start MSW
java  $appdMeSW -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 9191 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &


sleep $APP_START_DELAY
#Start AKAMAI
java $debugAkamai $jettyAkamai -jar ../../../../jetty-appchain/target/dependency/jetty-runner.jar $jmxEnable --port 9292 "$APPCHAIN_HOME"/target/test.war >> /tmp/server.log &

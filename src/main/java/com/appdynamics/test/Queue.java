...
public class Queue extends JMXBean {
	...
	public MBeanInfo getMBeanInfo() {
		...
		map.entrySet().stream().forEach(entry ->{
			MBeanAttributeInfo attr = new MBeanAttributeInfo(entry.getKey(), "", entry.getValue().getClass().getName(), true, true, entry.getValue().getClass().getName().toLowerCase().endsWith("boolean"));
			inf.add(attr);
		});
		return new MBeanInfo(Queue.class.getName(), "", inf.toArray(new MBeanAttributeInfo[inf.size()]), new MBeanConstructorInfo[0], new MBeanOperationInfo[0], new MBeanNotificationInfo[0]);
	}
